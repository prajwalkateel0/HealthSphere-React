...
const VALID = {
  ...
  exerciseIntensity: new Set(['low', 'moderate', 'high']),
...
const maps = {
  ...
  exercise_logs: (row) => ({
    ...
    exerciseType: text(row.exercise_type) || 'Exercise',
    durationMinutes: int(row.duration_minutes) || 0,
    caloriesBurned: num(row.calories_burned) || 0,
    intensity: cleanEnum(row.intensity, VALID.exerciseIntensity, 'moderate'),
    notes: text(row.notes),
    createdAt: date(row.created_at) || new Date(),
  }),
  water_logs: (row) => ({
  ...
  payments: (row) => ({
    ...
    stripePaymentIntentId: text(row.stripe_payment_intent_id) || `legacy-payment-${row.id}`,
...
const importOrder = [
  ...
  ['exercise_logs', prisma.exerciseLog],
